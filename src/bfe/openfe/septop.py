...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--conf_file",
        type=pathlib.Path,
        default=pathlib.Path("./septop_conf.yaml"),
...
def _default_lomap_scorer():
    try:
        from bfe.openfe import lomap_scorers
...
def _create_alchemical_network(ligand_network, protein, solvent, cofactors, protocol):
    transformations = []
    for mapping in ligand_network.edges:
        ...
        sysA = openfe.ChemicalSystem(sysA_dict, name=f"{mapping.componentA.name}")
        sysB = openfe.ChemicalSystem(sysB_dict, name=f"{mapping.componentB.name}")
            

        prefix = "septop_"  # prefix is only to exactly reproduce CLI
...
def _create_protocol(conf):
    ...
    # Restraint settings
    settings.complex_restraint_settings.anchor_finding_strategy = conf.anchor_finding_strategy
    settings.complex_restraint_settings.host_max_distance = conf.host_max_distance * offunit.nanometer
    settings.complex_restraint_settings.host_min_distance = conf.host_min_distance * offunit.nanometer
    settings.complex_restraint_settings.host_selection = conf.host_selection
    settings.complex_restraint_settings.rmsf_cutoff = conf.rmsf_cutoff * offunit.nanometer

    settings.solvent_restraint_settings.spring_constant = conf.solvent_restraint_spring_constant * offunit.kilojoule_per_mole / offunit.nanometer ** 2
    ...
    if conf.membrane_system:
        if conf.complex_box_vectors is None:
            raise ValueError("complex_box_vectors must be provided when membrane_system is True")

        settings.complex_solvation_settings.box_shape = None
        settings.complex_solvation_settings.solvent_padding = None
        settings.complex_solvation_settings.box_vectors = np.array(conf.complex_box_vectors) * offunit.nanometer
    else:
    ...
    settings.complex_simulation_settings.n_replicas = len(conf.complex_lambda_elec_A)
    # solvent
    ...
    settings.solvent_simulation_settings.n_replicas = len(conf.solvent_lambda_elec_A)
    ...
    # Lambda settings
    # --------------------
    settings.complex_lambda_settings.lambda_elec_A = conf.complex_lambda_elec_A
    settings.complex_lambda_settings.lambda_elec_B = conf.complex_lambda_elec_B
    settings.complex_lambda_settings.lambda_restraints_A = conf.complex_lambda_restraints_A
    settings.complex_lambda_settings.lambda_restraints_B = conf.complex_lambda_restraints_B
    settings.complex_lambda_settings.lambda_vdw_A = conf.complex_lambda_vdw_A
    settings.complex_lambda_settings.lambda_vdw_B = conf.complex_lambda_vdw_B

    settings.solvent_lambda_settings.lambda_elec_A = conf.solvent_lambda_elec_A
    settings.solvent_lambda_settings.lambda_elec_B = conf.solvent_lambda_elec_B
    settings.solvent_lambda_settings.lambda_restraints_A = conf.solvent_lambda_restraints_A
    settings.solvent_lambda_settings.lambda_restraints_B = conf.solvent_lambda_restraints_B
    settings.solvent_lambda_settings.lambda_vdw_A = conf.solvent_lambda_vdw_A
    settings.solvent_lambda_settings.lambda_vdw_B = conf.solvent_lambda_vdw_B

    return SepTopProtocol(settings=settings)
...
def main() -> None:
    ...
    if conf.membrane_system:
        ...
        box_vectors = np.array(conf.complex_box_vectors) * offunit.nanometer
    ...
    network = _create_alchemical_network(ligand_network, prot, solv, cofactors, protocol)
